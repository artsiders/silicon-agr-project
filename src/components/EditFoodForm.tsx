...
import { BiEdit, BiLoaderCircle, BiTrash } from "react-icons/bi";
...
export default function EditFoodForm(props: Props) {
  const { setVisible, food } = props
  ...
  const [formData, setFormData] = useState<Plat>(food);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (file) data.append("image", event.currentTarget.image.files[0])


    axiosURL.patch(`/restaurants/plats?platId=${food._id}&restaurantId=${restaurant._id}`, data, {
      headers: {
    ...
    >
      <div className="relative">
        ...
        <label htmlFor="image" className="p-5 overflow-hidden relative flex flex-col items-center justify-center w-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-tertiary hover:bg-secondary/10 dark:border-white/20 dark:focus:border-secondary dark:hover:border-gray-500 dark:hover:bg-tertiary/50">
          {!file && <span
            className="z-10 absolute top-2 right-2 btn-secondary !p-1 !rounded"
          >
            <BiEdit />
          </span>}
          <img
            src={file || `${import.meta.env.VITE_REACT_APP_API_URL}/uploads/plats/${food.image}`}
            alt="image-example"
      ...
      <div className="bg-red-100 dark:bg-tertiary p-5 rounded-md">
        <div className="flex flex-col justify-center items-center gap-3">
          <p>Supprimer définitivement le plat</p>
          <button
            className="btn bg-primary !w-full !justify-center"
            type="submit"
            disabled={isLoading}
          >
            {isLoading ? <>
              ...
              Supprimer <BiTrash className="text-lg ml-1" />
            </>}
          </button>
        </div>
      </div>
